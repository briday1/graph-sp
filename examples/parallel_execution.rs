...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let merge_config = NodeConfig::new(
        ...
        Arc::new(|inputs: &HashMap<String, PortData>| {
            ...
            let a = inputs.get("a").and_then(|d| match d {
                PortData::Int(v) => Some(*v),
                _ => None,
            }).unwrap_or(0);
            
            let b = inputs.get("b").and_then(|d| match d {
                PortData::Int(v) => Some(*v),
                _ => None,
            }).unwrap_or(0);
            
            let c = inputs.get("c").and_then(|d| match d {
                PortData::Int(v) => Some(*v),
                _ => None,
            }).unwrap_or(0);
    ...
    graph.add_edge(Edge::new("source", "value", "branch_a", "input")).unwrap();
    graph.add_edge(Edge::new("source", "value", "branch_b", "input")).unwrap();
    graph.add_edge(Edge::new("source", "value", "branch_c", "input")).unwrap();

    // Connect all branches to merger (fan-in)
    graph.add_edge(Edge::new("branch_a", "output", "merger", "a")).unwrap();
    graph.add_edge(Edge::new("branch_b", "output", "merger", "b")).unwrap();
    graph.add_edge(Edge::new("branch_c", "output", "merger", "c")).unwrap();
